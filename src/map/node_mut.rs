...
impl<'t, Token, Value> NodeMut<'t, Token, Value> {
    ...
    pub fn range(&'t self) -> NodeIter<'t, Token, Value> {
        NodeIter {
            trie: &self.trie,
    ...
    /// e.g. "app" → "apple"
    #[inline]
    pub fn starts_with(&'t self) -> PostfixIter<'t, Token, Value>
    where
        Token: Clone + Ord,
    {
...
impl<'t, Token, Value> NodeMut<'t, Token, Value> {
    ...
    #[inline]
    pub fn as_ref(&'t self) -> NodeRef<'t, Token, Value> {
        NodeRef {
            trie: self.trie,
...
impl<'t, Token, Value> TrieRef<'t, Token> for NodeMut<'t, Token, Value> {
